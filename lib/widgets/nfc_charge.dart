...
class NFCCharge extends StatefulWidget {
  final String amount;
  final VoidCallback onCancel;

  const NFCCharge({required this.amount, required this.onCancel, Key? key})
      : super(key: key);

  @override
  NFCChargeState createState() => NFCChargeState();
}

class NFCChargeState extends State<NFCCharge> {
  ...
  void startListening() async {
    ...
    Map<String, dynamic> dataToWrite = {'amount': widget.amount};
    tagData = await nfcController.writeCardData(dataToWrite);

    if (tagData != null) {
      setState(() {
        _nfcStatus = 'SUCCESS';
        _nfcStatusMessage = 'Tarjeta escrita correctamente';
      });
                      ...
                      child: Column(
                        children: [
                          ...
                          Padding(
                            ...
                            child: Center(
                              child: Text(
                                'Coloca la tarjeta y manténla cerca del dispositivo hasta que se complete la escritura',
                                style: TextStyle(
